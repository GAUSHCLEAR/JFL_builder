...
5. **导入和导出**
   * 导出：将 基础参数，曲率公式，弧段终点直径公式，曲率半径，弧段重点直径 序列化为 csv 字符串，供用户下载。
   * 导入：用户上传 csv 文件，解析并覆盖 上述参数，注意导入时应当解锁所有锁定状态。

6. **主循环 computeAndDraw()**  
...
* 响应式设计，适合手机、台式机屏幕。
* 曲率半径和弧段直径均显示3位小数
* 参数输入和图像显示区域分别布局，各自有滚动条，便于在调整参数时，分别观察不同的图像。
